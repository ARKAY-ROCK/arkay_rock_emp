...
@app.route('/users/logins', methods=['POST']) #/users/login
def logins():                                 #def login()
    users = mongo.db.users
    user_name = request.get_json()['user_name']
    password = hashlib.sha256(
        request.get_json()['password'].encode("UTF-8")).hexdigest()
    ...
    if (1 == 1):
        ...
        if (1 == 1):
            if(request.get_json()['password'] == 'admin'):
                ...
                access_token = create_access_token(identity={'user_name': request.get_json()[
                                                   'user_name'], 'device_name': 'device', 'Account': 2})
        ...
        if (1 == 1):
            if(request.get_json()['password'] == 'adminrollunroll'):
                #            access_token = create_access_token(identity = {'user_name':response['user_name'],'device_name' : response['device_name'] })
                access_token = create_access_token(identity={'user_name': request.get_json(
                )['user_name'], 'user_name': request.get_json()['user_name'], 'Account': 1})
                print(create_access_token)
...
def at_li():
    ...
    all_employees = client.list_database_names()
    try:
        db_date = request.get_json()['date'][-2:] + "/"+request.get_json()[
            'date'][5:7] + "/" + request.get_json()['date'][:4]
    except:
        db_date = "ab"
