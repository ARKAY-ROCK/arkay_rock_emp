...
def login():
    ...
    user_auth = user_authentication(request.get_json()['user_name'],request.get_json()['password'])
...
def get_overall_present_absent_overtime():
    ...
    get_report = attendence_data_mannual(emp_name,month,year).get_month_report()

    print("from class " , get_report)
    ...
    all_doc = client[request.get_json()['employee_name']].attendence.find(
        {'month': fetch_month+"_"+str(request.get_json()['year'])})
    print(all_doc)
    ...
    print(present_day)
    present_days = client[request.get_json()['employee_name']].attendence.find(
        {'month': fetch_month+"_"+str(request.get_json()['year'])})
    print(present_days)
...
def get_selected_day_details():
    ...
    print(new_date)
    details = client[request.get_json()['employee_name']
                     ].attendence.find({'date': new_date})
...
def add_mannual_attendence_new():
    ...
    if (str(request.get_json()['employee_name']) == 'all'):
        ...
        if (len(str(request.get_json()['in_time'])) == 5 and len(str(request.get_json()['out_time'])) == 5 and len(str(request.get_json()['delay'])) >= 1 and len(str(request.get_json()['advance'])) >= 1 and len(str(request.get_json()['overtime'])) >= 1):
            new_date = str(request.get_json()['date'][-2:]) + "/" + str(
                request.get_json()['date'][5:7])+"/" + str(request.get_json()['date'][:4])
            total_hours = abs(int(str(request.get_json()['out_time'])[
                              0:2])-int(str(request.get_json()['in_time'])[0:2]))
            total_min = abs(int(str(request.get_json()['out_time'])[
                            3:5])-int(str(request.get_json()['in_time'])[3:5]))
            total_hours_worked = datetime.strptime(
                str(total_hours)+":"+str(total_min), "%H:%M").time()
            month = months_list_post[int(
    ...
    if (str(request.get_json()['employee_name']) != 'all'):
        ...
        if (str(request.get_json()['in_time']) == "0" and str(request.get_json()['out_time']) == "0" and str(request.get_json()['delay']) == "0" and str(request.get_json()['advance']) == "0" and str(request.get_json()['overtime']) == "0"):
            ...
            new_date = str(request.get_json()['date'][-2:]) + "/" + str(
                request.get_json()['date'][5:7])+"/" + str(request.get_json()['date'][:4])
            client[str(request.get_json()['employee_name'])
    ...
    if (len(str(request.get_json()['in_time'])) == 5 and len(str(request.get_json()['out_time'])) == 5 and len(str(request.get_json()['delay'])) >= 1 and len(str(request.get_json()['advance'])) >= 1 and len(str(request.get_json()['overtime'])) >= 1):
        ...
        if(1 == 1):
            new_date = str(request.get_json()['date'][-2:]) + "/" + str(
                request.get_json()['date'][5:7])+"/" + str(request.get_json()['date'][:4])
            count = client[str(request.get_json()['employee_name'])].attendence.find(
                {'date': new_date}).count()
            print('count     ', count)
            ...
            if (count == 1):
                ...
                att_format.update(
                    {'overtime': prev_overtime+float(request.get_json()['overtime'])})
                print('new overtime', prev_overtime +
                      float(request.get_json()['overtime']))
                print(att_format)
                client[str(request.get_json()['employee_name'])
                       ].attendence.delete_one({'_id': new_date})
                client[str(request.get_json()['employee_name'])
...
def at_li():
    ...
    if (request.get_json()['key']=='date'):
        ext_attendence = ExtractAttendence(date={'date':request.get_json()['date']}).extract_attendence_date()
    if (request.get_json()['key']=='month_year'):
        ext_attendence = ExtractAttendence(month_year=request.get_json()['month_year']).extract_attendence_month_year()
...
@app.route('/employee_monthly_salary_month_years', methods=['PUT'])   ###main code
...
def reterive_userinfo():
    user_info = reterive_info(request.get_json()['user_name'])
    info = user_info.reterive_information()
    return info
