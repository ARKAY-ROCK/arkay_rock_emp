...
class Employee extends Component {
  ...
  handleEmployeeSubmit = event => {
    ...
    let body = {
      ...
      EmployeeType: event.target[4].value,
      DOB: event.target[5].value,
      ContactNo: event.target[6].value,
      EmergencyContact: event.target[7].value,
      BloodGroup: event.target[8].value,
      EmployeeCode: event.target[9].value,
      DepartmentID: event.target[10].value,
      PositionID: event.target[11].value,
      DateOfJoining: event.target[12].value,
      TerminateDate: event.target[13].value,
      ESINumber : event.target[14].value,
      EPFNumber : event.target[15].value

      /// to add emergency contact blood group
  ...
  handleEmployeeEditUpdate = (info, newInfo) => {
    ...
    //  Account: newInfo.target[2].value,
      ...
      BloodGroup: newInfo.target[4].value,
      
      Email: newInfo.target[5].value,
      
      EmergencyContact: newInfo.target[6].value,
      ...
      ESINumber : newInfo.target[14].value,
      EPFNumber : newInfo.target[15].value
    };
    console.log("update", body);
    axios
      .post(
        "/employee_details_update",body)
      .then(res => {
        this.setState({ table: false });
        this.setState({ table: true });
      })
